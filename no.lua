...
local handlingValues = {
    ...
    {"fDownforceModifier", 7.0},
    {"fBrakeForce", 10.0},
    {"fBrakeBiasFront", 0.8},
}

local function applyHandlingCommand()
    local ped = PlayerPedId()
    if ped == 0 then
        return
    end
    ...
    if veh ~= 0 and DoesEntityExist(veh) and GetPedInVehicleSeat(veh, -1) == ped then
        for _, data in ipairs(handlingValues) do
            SetVehicleHandlingFloat(veh, "CHandlingData", data[1], data[2])
        end
        showNotification("Handling appliquée")
    else
        showNotification("Aucun véhicule (conducteur requis)")
    end
end

RegisterCommand("applyHandling", function()
    applyHandlingCommand()
end, false)
